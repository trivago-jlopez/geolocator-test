"""
Tests the Google Places geocoding services.
"""
import geocoder
import pytest
...
def test_google_places_service():
    providers.GooglePlaces()


def test_google_places_call(setup_accommodation, setup_key, setup_response, mocker):
    service = providers.GooglePlaces()
...
def test_google_places_call_with_guess(setup_accommodation_with_guess, setup_key, setup_response, mocker):
    service = providers.GooglePlaces()
    ...
    assert geocoder.google.call_count == 1
    geocoder.google.assert_called_with(
        setup_accommodation_with_guess.name,
        components='administrative_area:{administrative_area}|country:{country_code}|locality:{city}|postal_code:{postal_code}'.format(
...
def test_google_places_iterative_addresses(setup_accommodation, setup_key, mocker):
    """
...
def test_google_places_rate_limiting(setup_accommodation, mocked_key_handler, mocker):
    service = providers.GooglePlaces()
    ...
    geocoder.google.call_count == 3


def test_google_places_failure(setup_accommodation, mocked_key_handler, mocker):
    service = providers.GooglePlaces()
...
def test_google_places_invalid_request(setup_accommodation, mocked_key_handler, mocker):
    service = providers.GooglePlaces()
