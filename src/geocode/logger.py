...
from geocode.helpers import GeocoderError
...
# Logging (CloudWatch)
LOGGER = logging.getLogger('trvcoder')
LOGGER.setLevel(logging.INFO)
for handler in logging.root.handlers:
    handler.setFormatter(JsonFormatter())

if os.environ.get('ENVIRONMENT') not in ('production', 'development'):
...
def log_status(level, status, **kwargs):
    LOGGER.log(
        level, {
            **kwargs,
            'status': status
